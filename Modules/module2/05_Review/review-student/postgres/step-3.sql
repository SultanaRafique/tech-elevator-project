...
SELECT name, open_from_mm, open_to_mm, daily_fee FROM campground WHERE daily_fee::numeric::float8   < 100.00;

-- select name and daily fee of the campgrounds where the campground is open all year long
SELECT name, daily_fee from campground WHERE (open_to_mm::numeric - open_from_mm::numeric + 1) = 12;
...
SELECT reservation_id, site_id, name, from_date, to_date FROM reservation WHERE extract(month from from_date)  = 6;
...
SELECT SUM(reservation_id) FROM reservation;

-- select reservation id, site id, name of the reservations where site id is in the list 9, 20, 24, 45, 46
SELECT reservation_id , site_id, name FROM reservation WHERE reservation_id IN (9, 20, 24, 45, 46);

...
*/
		...
		function frontAgain(str) {
			...
			let frontSubString = str.substring(0,2);
			let endSubString = str.substring(str.length - 2 ,str.length);

			if(frontSubString === endSubString){
				return true;
			} else {
...
*/
		function filterMultiplesOfX( array, number){
			...
			let multipleOfNumber = [];

			for(let i = 0; i < array.length; i++){

				if(array[i] % number === 0){
					multipleOfNumber.push(array[i]);
				}
			}

  ...
  <div class="container">
    <table id="tblUsers">
      ...
      <tbody>
        <tr>
          <td>npm run 
            <!--<input type="checkbox" id="selectAll"  name = "selectedUserIDs" value="user.id"  v-model="selectedUserIDs"/>-->
             <input type="checkbox" id="selectAll" v-on:change="selectAllAction($event)"/> 
          </td>
          <td>
...
Bind the checked value to if the user's ID is in the selectedUserIDs array. -->
      ...
      <input type="checkbox" v-bind:id="user.id" v-bind:value="user.id" v-model="selectedUserIDs"/>
          ...
          <td>
            ...
            <button v-on:click="flipStatus(Number(user.id))" class="btnEnableDisable" >
...
export default {
  ...
  methods: {
    ...
    selectedUser(id){
      ...
      return this.selectedUserIDs.forEach(selectId => {
          return selectId === id;
      });
    },
  ...
  computed: {
    filteredList() {
     ...
     actionButtonDisabled(){
      if(this.selectedUserIDs != undefined && this.selectedUserIDs.length === 0){
        return true;
      } 
      return false;
    }

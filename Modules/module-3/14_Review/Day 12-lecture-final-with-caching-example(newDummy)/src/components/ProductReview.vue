  ...
  <div class="main">
    ...
    <div class="well-display">
      ...
      <div class="well" v-on:click="filter=1">
        <span class="amount">{{ numberOfStarReviews(1) }}</span>
        1 Star Review{{ numberOfStarReviews(1) === 1 ? '' : 's' }}
      </div>

      <div class="well" v-on:click="filter=2">
        <span class="amount">{{ numberOfStarReviews(2)  }}</span>
        2 Star Review{{ numberOfStarReviews(2)  === 1 ? '' : 's' }}
      </div>
    ...
    <a href="#" v-show="!showForm" v-on:click.prevent="showForm=true">Show Form</a>
...
export default {
  ...
  methods: {
    ...
    numberOfStarReviews(stars) {
      return this.reviews.reduce((currentCount, review) => {
        return currentCount + (review.rating === stars);
      }, 0);
    },
    getFilteredReviews() {
      console.log('getFilteredReviews')
      return this.filter === 0 ? this.reviews : 
        this.reviews.filter( (review) => {
          return review.rating === this.filter
        });

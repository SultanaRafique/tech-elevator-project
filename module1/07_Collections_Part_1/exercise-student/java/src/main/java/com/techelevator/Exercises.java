...
public class Exercises {
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for (int i = 0; i < len; i++){
			int stringLen = stringArray[i].length();
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for(int item : intArray){
			double doubleItem = item / 2.0;
			doubleList.add(doubleItem);
		}
		return doubleList;
	}
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for( int item : integerArray){
			boolean multi5n3 = ((item % 3) == 0) && ((item % 5) == 0);
			boolean multi5 = ((item % 5) == 0);
			boolean multi3 = ((item % 3) == 0);
			...
			if(multi5n3){
				...
				fizzBuzz.add(String.valueOf(item));
			}
		}
		return fizzBuzz;
	}

...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {
		boolean isFirstLast6 = nums[0] == 6 || nums[nums.length-1] == 6;
	...
	public boolean sameFirstLast(int[] nums) {
		...
		boolean len1Nmore = numsLen >= 1;
		boolean firstLastSame = nums[0] == nums[nums.length - 1];

		if(len1Nmore && firstLastSame){
			return true;
		}
		return false;
	}
	...
	public boolean commonEnd(int[] a, int[] b) {
		...
		boolean aLen1Nmore = aLen >= 1;
		boolean bLen1Nmore = bLen >= 1;
		boolean aNb1NMore = aLen1Nmore && bLen1Nmore;

		boolean aNbFirstSame = a[0] == b[0];
		boolean bNbLastSame = a[a.length - 1]== b[b.length - 1];
		boolean aNbSameFirstOrLast = aNbFirstSame || bNbLastSame;



		if(aNb1NMore && aNbSameFirstOrLast){
			return true;
		}
	...
	public int[] rotateLeft3(int[] nums) {
		...
		int[] leftRotateArr = new int[numsLen ];

		for (int i = 0; i < nums.length ; i++){
			leftRotateArr[i] = nums[(i + 1) % numsLen];
		}
	...
	public int[] maxEnd3(int[] nums) {
		 ...
		 int maxEnd = nums[0] > nums[nums.length - 1] ? nums[0] : nums[nums.length - 1];

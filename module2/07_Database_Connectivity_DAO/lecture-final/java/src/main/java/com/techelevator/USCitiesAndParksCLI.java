...
public class USCitiesAndParksCLI {
    ...
    private void listStateInfo() {
        State state = promptForState();
        if (state != null) {
            System.out.println();
            System.out.println(String.format("%s %s", state.getStateAbbreviation(), state.getStateName()));
            System.out.println();
        }
    }
    ...
    private void deletePark(Park parkToDelete) {
        parkDao.removeParkFromState(parkToDelete.getParkId(), parkToDelete.getStateAbbreviation());
    ...
    private void displayMenu() {
        System.out.println("0. Get state: ");
    ...
    private Park promptForPark() {
        ...
        while (parks.size() == 0) {
            ...
            if (parks.size() == 0) {
                ...
                for (Park park : parks) {
                    park.setStateAbbreviation(state.getStateAbbreviation());
                }
            }
        }

...
-- examples related to IN
SELECT * FROM city WHERE state_abbreviation = 'PA' OR state_abbreviation = 'OH';
SELECT * FROM city WHERE state_abbreviation IN ('PA', 'OH', 'WI');
...
-- The names and census regions of all states (and territories and districts) not in the Midwest region (including those that ar eno in Midwest census region)
SELECT * FROM state
...
SELECT park_name, area AS area_in_meters FROM park;
...
SELECT  city_name FROM city ORDER BY city_name;
...
SELECT * FROM park;
SELECT park_name, date_established FROM  PARK WHERE date_established < date '1900-01-01'; 

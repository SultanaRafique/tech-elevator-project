...
//              BASE_URL + "hotels/" + reservation.getHotelID() + "/reservations",
//              HttpMethod.POST, entity, Reservation.class);

      // Don't actually need exchange for POST unless you want status, headers in additon to object being sent back
      Reservation response = restTemplate.postForObject(BASE_URL + "hotels/" + reservation.getHotelID() + "/reservations", entity, Reservation.class);
      if (response != null) {
  ...
  public Hotel[] listHotels() throws HotelServiceException {
    ...
    try {
      ResponseEntity<Hotel[]> response =
              restTemplate.exchange(BASE_URL + "hotels", HttpMethod.GET, makeAuthEntity(), Hotel[].class);
      hotels = response.getBody();
//      Hotel[] ressult = restTemplate.getForObject(BASE_URL + "hotels", Hotel[].class);
    } catch (RestClientResponseException ex) {
      throw new HotelServiceException(ex.getRawStatusCode() + " : " + ex.getResponseBodyAsString());
    }
    return hotels;
